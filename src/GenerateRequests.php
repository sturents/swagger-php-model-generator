...
class GenerateRequests extends ClassGenerator {
	...
	private function swaggerRequest(){
	...
	public function generate(string $file_path){
		...
		$swagger_request = $this->swaggerRequest();
		...
		foreach ($api['paths'] as $path => $path_details){
			list($path_no_params,$params) = $this->pathParams($path);
			...
			foreach ($path_details as $method => $method_details){
				...
				foreach ($params as $param){
					...
					$class->addProperty($param);
					$constructor->addBody("\$this->$param = \$$param;");
				}
				$class->addProperty('param_names')->setStatic(true)->setValue($params)->setVisibility('protected');
	 ...
	 * @return array
	 */
	private function pathParams(string $path){
		$path = substr($path, 1);
